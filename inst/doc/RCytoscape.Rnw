...
Here we create a 3-node graphNEL in R, send it to Cytoscape for display and layout.  For the sake of simplicity, no node attributes, no edges, and no vizmapping are included.  Those topics are covered in subsequent examples.
